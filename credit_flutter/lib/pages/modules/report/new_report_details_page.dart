...
/// @Description: 芝麻数据-报告详情页
class NewReportDetailsPage extends StatefulWidget {
  String reportId;
  String authId;

  NewReportDetailsPage({Key? key, required this.reportId, required this.authId})
      : super(key: key);

  @override
  State<NewReportDetailsPage> createState() => _NewReportDetailsPageState();
}

class _NewReportDetailsPageState extends State<NewReportDetailsPage> {
  //名称
  String name = "";
  ...
  void initState() {
    //初始化主页面
    super.initState();
    UmengCommonSdk.onPageStart("report_details_page");
    UmengCommonSdk.onEvent(
        "ReportDetails", {"state": "start", "time": "${DateTime.now()}"});
    ...
    // prefs.then((sp) {
    //   bool firstOpen = sp.getBool(FinalKeys.FIRST_OPEN_REPORT_DETAILS) ?? false;
    //   if (!firstOpen) {
    //     isPop = true;
    //   } else {
    //     String reportIds = sp.getString(FinalKeys.POP_REPORT_ID) ?? "";
    //     String popTime = sp.getString(FinalKeys.REPORT_DETAILS_TIMING) ?? "";
    //     int days = StringTools.getTimeDifference(popTime);
    //     if (!reportIds.contains(widget.reportId) && days > 7) {
    //       isPop = true;
    //     }
  ...
  void dispose() {
    super.dispose();
    UmengCommonSdk.onPageEnd("report_details_page");
    UmengCommonSdk.onEvent(
        "ReportDetails", {"state": "end", "time": "${DateTime.now()}"});
  }
    ...
    scrollController.addListener(() {
      if (scrollController.position.pixels >=
        ...
        if (isPop) {
          ...
          box.put(FinalKeys.POP_REPORT_ID, idStr);
          box.put(FinalKeys.FIRST_OPEN_REPORT_DETAILS, true);
          // Future<SharedPreferences> prefs = SharedPreferences.getInstance();
          // prefs.then((sp) {
          //   sp.setString(
          //       FinalKeys.REPORT_DETAILS_TIMING, StringTools.getNowTime());

          //   String idStr = sp.getString(FinalKeys.POP_REPORT_ID) ?? "";
          //   if (idStr.isNotEmpty) {
          //     idStr = "$idStr,${widget.reportId}";
          //   }
          //   sp.setString(FinalKeys.POP_REPORT_ID, idStr);

          //   sp.setBool(FinalKeys.FIRST_OPEN_REPORT_DETAILS, true);
          // });
  ...
  void _initData(String reportId) async {
    ReportDetailsManager.getReportData(reportId, (object) {
      try {
        var data = ReportDataBean.fromJson(object);

        _loadData(data);
      } catch (_) {
        // phoneReport?reportId=4202309010025858730&authId=5207&token=
        var url =
            "http://www.nowcheck.cn/phoneReport?reportId=${widget.reportId}&authId=${widget.authId}&token=${Golbal.token}";
        Log.e(url);
        Log.i(Golbal.token);
        //跳转并关闭当前页面
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => WebViewPage("报告详情", url),
          ),
        );
      }
    });
  ...
  void _loadData(ReportDataBean data) {
    ...
    //创建时间
    var time = DateTime.fromMillisecondsSinceEpoch(data.reportInfo.createTime);
    ...
    if (inputMap.age == 0) {
      var registeredResidenceInfo = detail.registeredResidenceInfo;
      var itemData102L = registeredResidenceInfo?.itemData102 ?? [];
      if (itemData102L.isNotEmpty) {
        for (ItemData102 item in itemData102L) {
          if (item.itemPropName == "age") {
            age = item.itemPropValue;
          }
        }
      }
    } else {
    ...
    if (itemData452L.isNotEmpty) {
      ...
      for (var i = 0; i < itemData764.itemPropValue.length; i++) {
        ...
        peopleWhoLoseCreditList
            .add(ProfessionalCertificateBean("法院失信被执行人${i + 1}", content, ""));
      }
    }
    ...
    if (detail.onlineLoanBlacklist != null) {
      //网贷黑名单
