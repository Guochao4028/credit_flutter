...
class ChangePasswordPage extends StatefulWidget {
  final PageType type;
  const ChangePasswordPage({required this.type, Key? key}) : super(key: key);
...
class _ChangePasswordPageState extends State<ChangePasswordPage> {
  ...
  void _pTapAction() {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: ((context) => ForgotPasswordPage(type: widget.type)),
      ),
    );
  }
  ...
  void _next() {
    ...
    if (widget.type == PageType.password) {
      ...
      MineManager.checkPayPassword(md5Str, (message) {
        if (message.isSuccess) {
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: ((context) => SetupNewPasswordPage(
